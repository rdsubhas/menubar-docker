...
let buildMainMenu = Promise.coroutine(function *() {
  ...
  trayIcon.setImage(path.resolve(__dirname, '../images/menu' + (selectedRunning ? 'Active' : 'Template') + '.png'))
...
let buildMachinesMenu = Promise.coroutine(function *(machines, selectedMachine) {
  ...
  for (let machine of machines) {
    ...
    menu.append(new MenuItem({
      ...
      icon: path.resolve(__dirname, '../images/circle' + (isRunning ? 'Green' : 'Red') + '.png'),
