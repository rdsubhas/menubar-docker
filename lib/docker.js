...
exports.machines = function () {
  return child_process.execFileAsync('docker-machine', ['ls']).spread(function (stdout, stderr) {
    return parseColumns(stdout).map(function (machine) {
      machine.isRunning = machine.ACTIVE && machine.ACTIVE === '*'
      return machine
    })
  })
}
...
exports.killMachine = function (machine) {
  return child_process.execFileAsync('docker-machine', ['kill', machine.NAME]).catch(nullFn)
}

exports.removeMachine = function (machine) {
  return child_process.execFileAsync('docker-machine', ['rm', machine.NAME]).catch(nullFn)
}
